...
"""
    ...
    def _apply_fraud_routing(self, data: dict[str, Any], source_text: str) -> None:
        """Ensure fraud/risk queries surface BOTH IBM transactions and Unisys shopping.

        Without this, the default intent extractor returns only ``shopping`` and the
        context resolver never reaches IBM, so the fraud assessment cannot score
        actual transactions.
        """
