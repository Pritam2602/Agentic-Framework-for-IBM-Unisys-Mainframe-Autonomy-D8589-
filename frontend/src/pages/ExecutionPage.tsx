...
import { PipelineVisualization, IntentPanel, ContextPanel, PlannerPanel, ReasoningPanel, ObservabilityPanel } from "@/components/pipeline";
...
const panelNav: Array<{
  ...
  { id: "observability", label: "Observability", icon: SignalIcon, description: "Inspect request IDs, stage timings, and domain health checks." },
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-4">
          ...
          <p className="mt-2 text-3xl font-semibold text-red-200">{summary.verdict_counts?.likely_fraud ?? 0}</p>
        </div>
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4">
          <p className="text-xs uppercase tracking-[0.18em] text-amber-200/80">Suspicious</p>
          <p className="mt-2 text-3xl font-semibold text-amber-200">{summary.verdict_counts?.suspicious ?? 0}</p>
        </div>
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4">
          <p className="text-xs uppercase tracking-[0.18em] text-emerald-200/80">Genuine</p>
          <p className="mt-2 text-3xl font-semibold text-emerald-200">{summary.verdict_counts?.genuine ?? 0}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <KeyValueTable
          rows={[
            ...
            ["High / Medium / Low", `${summary.risk_band_counts?.high ?? 0} / ${summary.risk_band_counts?.medium ?? 0} / ${summary.risk_band_counts?.low ?? 0}`],
          ]}
        />
        <KeyValueTable
          rows={[
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800">
          <div className="border-b border-slate-800 bg-slate-900/80 px-4 py-3">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Sorted by risk score. Every verdict cites the rules that fired.
            </p>
          </div>
...
const renderPanel = (panel: ControlCenterPanel) => {
  ...
  if (panel === "observability") {
    return <ObservabilityPanel trace={store.trace} observability={store.observability} />;
  }
