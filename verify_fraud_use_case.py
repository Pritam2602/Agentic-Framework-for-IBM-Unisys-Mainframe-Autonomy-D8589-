...
from app.api.pipeline import PipelineRequest, run_pipeline
...
def step_pipeline() -> dict:
    ...
    response = asyncio.run(run_pipeline(request))
    federation = response.federation_intelligence or {}
    top_view = (federation.get("top_view") or {}).get("view_id")
    print(f"  pipeline_stage:  {response.pipeline_stage}")
